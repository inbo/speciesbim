...
def populate_vernacular_names(conn, empty_only):
    ...
    if empty_only:
        taxa_selection_sql = """SELECT *
                                FROM taxonomy
                                WHERE NOT EXISTS (SELECT vernacularname."taxonomyId" FROM vernacularname WHERE taxonomy.id = vernacularname."taxonomyId")"""
    else:
        taxa_selection_sql = """SELECT * FROM taxonomy;"""

    cur = execute_sql_from_jinja_string(conn, sql_string=taxa_selection_sql, dict_cursor=True)
